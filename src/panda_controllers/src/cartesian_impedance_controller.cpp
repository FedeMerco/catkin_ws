...
#include <panda_controllers/cartesian_impedance_controller.h>
...
bool CartesianImpedanceController::init(hardware_interface::RobotHW* robot_hw,
  ...
  sub_equilibrium_pose_ = node_handle.subscribe(
      "equilibrium_pose", 1, &CartesianImpedanceController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  sub_desired_stiffness_ = node_handle.subscribe(
      "desired_stiffness", 1, &CartesianImpedanceController::desiredStiffnessCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
...
void CartesianImpedanceController::starting(const ros::Time& /*time*/) {
  ...
  //Qui si salva il robot state sfruttando il puntatore state_handle, in particolare più avanti usa dq e q, ossia velocità e posizioni dei giunti 
  franka::RobotState initial_state = state_handle_->getRobotState();
  
  // get jacobian
  // Allo stesso modo qui usa il secondo puntatore per ricavare il Jacobiano riferito all'EE 
  std::array<double, 42> jacobian_array =
      model_handle_->getZeroJacobian(franka::Frame::kEndEffector);
  
  // convert to eigen
  // I valori ottenuti dai puntatori sono array double, qui si rimappano questi array come matrice e vettori colonna
...
void CartesianImpedanceController::desiredStiffnessCallback(
  ...
  // Al momento i miei movimenti riguardano esclusivamente l'asse z quindi posso commentare x e y
  // cartesian_stiffness_target_(0,0) = msg->vector.x;
  // cartesian_stiffness_target_(1,1) = msg->vector.y;

  // Oltre commentare x e y, potrebbe essere corretto specificarle a 200 in modo da non essere modificate dal setIdentity qui sopra
  cartesian_stiffness_target_(0,0) = 200;
  cartesian_stiffness_target_(1,1) = 200;
  ...
  cartesian_damping_target_(0,0) = 2.0 * sqrt(200);
  cartesian_damping_target_(1,1) = 2.0 * sqrt(200);
  cartesian_damping_target_(2,2) = 2.0 * sqrt(200);
  
  cartesian_damping_target_(3,3) = 2.0 * sqrt(20);
  cartesian_damping_target_(4,4) = 2.0 * sqrt(20);
  cartesian_damping_target_(5,5) = 2.0 * sqrt(20);
}
...
PLUGINLIB_EXPORT_CLASS(panda_controllers::CartesianImpedanceController,
