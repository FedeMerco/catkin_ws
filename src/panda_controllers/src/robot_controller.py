...
TABLE_HEIGHT = 0.3   # 0.016 -- Grasso sottile; 0.021 -- Arteria; 0.022 -- Grasso spesso; 0.017 -- Vena
...
#        move_group_robot.set_planner_id("PTP")
        ...
        self.table_height = TABLE_HEIGHT - 0.005   # --ori 0.010
    ...
    def set_bias(self):
        ...
        try:
            response = self.ft_client(srv_ft.cmd)
...
#    robot_controller_node.move_group_robot.set_path_constraints(goal_c)
    ...
    while True:
        ...
        if choice == "":
            ...
            target = PoseStamped()
            target.pose.position.x = 0.30707093243261097
            target.pose.position.y = -0.00004975443320201987
            target.pose.position.z = 0.4871414711004457
            quat = R.from_matrix(np.identity(3)).as_euler("xyz", degrees=True)
            quat = R.from_euler('xyz', [quat[0]+180, quat[1], quat[2]], degrees=True).as_quat()
            ...
            table = PoseStamped()
            table.pose.position.x = 0.30707093243261097
            table.pose.position.y = -0.00004975443320201987
            ...
            #resp = robot_controller_node.stop_save_data()
